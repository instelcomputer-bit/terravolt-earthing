...
6. Set the public `SITE_URL` build environment variable to the final origin, for example `https://your-domain.com`. This adds a canonical URL, `og:url` and an absolute social-sharing image URL at build time. Without a known domain, these are intentionally omitted. `.env.example` documents the setting; no secrets are needed. Serve at the domain root.
